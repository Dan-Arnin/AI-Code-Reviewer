...
class OCIGenAIClient:
    ...
    def chat(self, prompt: str) -> str:
        ...
        Raises:
            RuntimeError: If the OCI API call fails.
        """
        ...
        try:
            ...
            log.error(
                "OCI Service Error during chat call.\n"
                "  HTTP STATUS     : %s\n"
                "  OCI ERROR CODE  : %s\n"
                "  MESSAGE         : %s\n"
                "  WHAT TO DO      : %s",
            ...
            raise RuntimeError(f"OCI ServiceError {exc.status} ({exc.code}): {exc.message}") from exc

        except Exception as exc:
            log.error(
                "Unexpected error during OCI chat call.\n"
                "  WHAT WENT WRONG : %s\n"
